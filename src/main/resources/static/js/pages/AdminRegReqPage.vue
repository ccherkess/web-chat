<template>
    <div class="container">
        <h1 class="header-label">Список заявок на регистрацию</h1>
        <div class="users-container" id="scroll">
            <div v-for="user in users" class = "user-item">
                ...
                <button v-if="user.name !== 'admin'" class="button" @click="lookUser(user.id, true)">Добавить</button>
                <button v-if="user.name !== 'admin'" class="button" @click="lookUser(user.id, false)">Удалить</button>
            </div>
        </div>
    </div>
</template>
...
<script>
    export default {
        ...
        methods: {
            ...
            lazyFetchUsers() {
              fetch(`/registration/list/25/${this.users[this.users.length - 1].id}`, {
                method: 'GET',
                headers: {
                  'Content-Type': 'application/json'
                }
            ...
            lookUser(id, solution) {
                 fetch(`/registration/solution/${id}/${solution}`, {
                   method: 'POST',
                   headers: {
                     'Content-Type': 'application/json'
                   }
